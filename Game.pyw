...
class GUI:
    ...
    def _showAbout(self):
        ...
        tk.Label(
            master = window,
            text = 'Author 2021, "Abhijeet Verma"\nThis software is licensed under -',
            font = self.font,
            justify = tk.LEFT
        ).grid(row = 0, column = 0, padx = padx, pady = pady, sticky = tk.W)
        ...
        tk.Button(
            ...
            text = "identity ->",
            font = ('Courier', 8),
            command = lambda: webbrowser.open(url = 'https://youtube.com/channel/UCzfmI9iqCH7h9RJUNYJ67sw')
        ).grid(row = 0, column = 1, padx = padx, pady = pady, sticky = tk.W)

        tk.Button(
            master = window,
            text = ' MIT License ',
            font = self.font,
            command = lambda: webbrowser.open(url = 'https://opensource.org/licenses/mit-license.php')
        ).grid(row = 1, column = 0, padx = padx, pady = pady, sticky = tk.W)
    ...
    def _applyButtonAction(self):
        ...
        try:
            ...
            if(messagebox.askyesno(title = 'Apply', message = 'The simulator will be reset. Are you sure to apply changes?')):
